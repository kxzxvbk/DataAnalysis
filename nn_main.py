...
class NNModel:
    ...
    def fit(self, x, y):
        ...
        optim = torch.optim.Adam(self.model.parameters(), lr=1e-5, weight_decay=1e-4)
        dl = DataLoader([(x[idx], y[idx]) for idx in range(x.shape[0])], batch_size=32, shuffle=True)
        for _ in tqdm(range(30)):
            for _, data in enumerate(dl):
                xx, yy = data
                y_pred = self.model(xx.float())
                ls = loss(y_pred, yy.float().unsqueeze(-1))
                optim.zero_grad()
                ls.backward()
                optim.step()

    def predict(self, x):
        return self.model(torch.from_numpy(x).float()).item()
...
def eval(model, dataset):
    ...
    for i in dataset['age'].keys():
        ...
        xx, yy = cvt_raw_data(sample)
        total_preds.append(model.predict(xx))
